...
def create_chart_image(date_str, time_str, lat, lon, timezone, name="Client"):
    ...
    for i, (symbol, color) in enumerate(ZODIAC_SIGNS):
        ...
        ax.text(0.98 * np.cos(mid_angle), 0.98 * np.sin(mid_angle), symbol, ha='center', va='center', fontsize=16)

    for pname, angle in positions.items():
        rad = np.deg2rad(angle)
        x, y = 0.75 * np.cos(rad), 0.75 * np.sin(rad)
        ax.plot(x, y, 'o', color='black')
        ax.text(x, y, PLANET_SYMBOLS[pname], fontsize=13, ha='center', va='center', color='black')

    ax.text(-1.25, 1.2, name, ha='left', va='center', fontsize=16, weight='bold')

    table_text = f"Date: {date_str} {time_str}\nLat: {lat}  Lon: {lon}  TZ: GMT{timezone:+.1f}h\n\n"
    for pname, angle in positions.items():
        deg = int(angle)
        min = int((angle - deg) * 60)
        table_text += f"{PLANET_SYMBOLS[pname]} {pname:<8}: {deg}°{min:02}'\n"
